...
前面说过，一致性校验算法需要尽可能多的去重用已经创建的节点。因为目前元素的`type`都是代表HTML中的标签名，所以如果同一位置的前后两次渲染的元素的类型一样就代表是同一类元素，对应的这个dom节点就可以被重用。下面我们在`reconcile`中增加判断前后两次渲染的元素类型是否相同的功能，相同的话执行更新操作，否则是新建或者替换。
